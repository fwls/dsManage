...
const generateJobs = async (pushDataToChannel, WebSocket) => {
  const datasets = await knex.table('data_channel_data_sets as a')
    ...
    .select('a.data_channel_id', 'a.data_set_id', 'a.id as data_channel_data_sets_id',
  ...
  for (const dataset of datasets) {
    ...
    dataSetJobs[dataset.data_set_id] = schedule.scheduleJob(dataset.push_cron, async function () {
      ...
      pushDataToChannel(`channel${dataset.data_channel_id}`, { data_channel_data_sets_id: dataset.data_channel_data_sets_id, timestamp: new Date(), result }, WebSocket);
    });
  }
}
