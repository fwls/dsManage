...
router.get('/sets/list', verifyToken, async (req, res) => {
  try {
    const query = knex(tableNameSets).select("id", "name", "status", "created_at");
    const { name } = req.query;
    if (name) {
      query.where('name', 'like', `%${name}%`);
    }
    ...
    const results = await query.whereNull('deleted_at').offset((req.query.page || 1) - 1).limit(req.query.pageSize || 10);
    const totalCount = await knex(tableNameSets).whereNull('deleted_at').count('id as count')
      .where(name ? { name: `%${name}%` } : {});
...
router.get('/sets/:id', verifyToken, async (req, res) => {
  try {
    const id = req.params.id;
...
router.get('/sets/add', verifyToken, async (req, res) => {
  const data = {
    name: req.body.name,
    type: req.body.type,
...
router.get('/sets/edit', verifyToken, async (req, res) => {
  const { id } = req.body
  const updateFields = ['name', 'type', 'data_channel_id', 'status']
  const data = await knex(tableNameSets).where('id', id).update(
    pick(req.body, updateFields),
    '*'
  ).catch(error => {
    res.json({
...
router.get('/sets/delete', verifyToken, async (req, res) => {
  try {
    const { id } = req.body
